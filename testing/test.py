...
if args.dataset.lower() == 'qnrf':
    ...
    dataset = crowd.Crowd_sh(os.path.join(data_path, 'test_data'), crop_size, 8, method='test')
else:
...
for inputs, count, name in dataloader:
    ...
    f = open("img_name_targ_pred.txt","a+")
    f.write(str(int(float(name[0].replace('IMG_','').replace(".jpg","").replace("img_","").replace('u',''))))+","+str(count[0].item())+","+str(torch.sum(outputs).item())+"\n")
    f.close()
